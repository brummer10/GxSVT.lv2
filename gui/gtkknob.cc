...
#define GTK_AMPEGSVT_KNOB_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), GTK_TYPE_AMPEGSVT_KNOB, GtkAmpegSVTKnobPrivate))

G_DEFINE_TYPE(GtkAmpegSVTKnob, gtk_knob, GTK_TYPE_RANGE);

typedef struct _GtkAmpegSVTKnobPrivate GtkAmpegSVTKnobPrivate;

struct _GtkAmpegSVTKnobPrivate
{
    int knob_x;
    int knob_y;
    int knob_step;
...
static void knob_expose(GtkWidget *widget, int knob_x, int knob_y, GdkEventExpose *event, int arc_offset)
{
    /** check resize **/
    int grow;
    GtkAmpegSVTKnobPrivate *priv = GTK_AMPEGSVT_KNOB_GET_PRIVATE(widget);
    ...
    if (priv->model != 3) {
        ...
        GtkAmpegSVTKnob *knob = GTK_AMPEGSVT_KNOB(widget);
...
{
    g_assert(GTK_IS_AMPEGSVT_KNOB(widget));
    GtkAmpegSVTKnob *knob = GTK_AMPEGSVT_KNOB(widget);
    GtkAmpegSVTKnobPrivate *priv = GTK_AMPEGSVT_KNOB_GET_PRIVATE(knob);
...
{
    GtkAmpegSVTKnob *knob = GTK_AMPEGSVT_KNOB(object);
...
/****************************************************************
 ** init the GtkAmpegSVTKnobClass
 */

static void gtk_knob_class_init (GtkAmpegSVTKnobClass *klass)
{
...
//--------- connect the events with funktions
    ...
    g_type_class_add_private(obj_class, sizeof (GtkAmpegSVTKnobPrivate));
}
...
static void gtk_knob_init (GtkAmpegSVTKnob *knob)
{
...
{
    GtkWidget *widget = GTK_WIDGET( g_object_new (GTK_TYPE_AMPEGSVT_KNOB, NULL ));
