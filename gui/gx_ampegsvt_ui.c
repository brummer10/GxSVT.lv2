...
static LV2UI_Handle instantiate(const LV2UI_Descriptor*   descriptor,
    ...
    *widget = NULL;
    ...
    ui->hbox = gtk_hbox_new(TRUE, 24);
    gtk_container_set_border_width(GTK_CONTAINER(ui->pbox),5);
    gtk_container_set_border_width(GTK_CONTAINER(ui->hbox),45);
    ui->vbox = gtk_vbox_new(TRUE, 0);

    gtk_box_pack_start(GTK_BOX(ui->pbox), ui->box, TRUE, TRUE, 0);
    gtk_box_pack_start(GTK_BOX(ui->box), ui->hbox, TRUE, TRUE, 10);
