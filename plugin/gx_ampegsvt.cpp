...
#include "simplecabsim3.cc"    // dsp class generated by faust -> dsp2cc
...
void Gx_ampegsvt_::init_dsp_(uint32_t rate)
{
  ...
  cabsim->set_samplerate(rate, cabsim); // init the DSP class
  for(uint32_t i=0; i<TONE_COUNT; i++) {
...
void Gx_ampegsvt_::run_dsp_(uint32_t n_samples)
{
  ...
  if (cabswitch_ != static_cast<uint32_t>(*(cabswitch))) {
  ...
  if (needs_ramp_down) {
    ...
    if (ramp_down <= 0.0) {
      ...
      if (cabmodel != static_cast<uint32_t>(cabswitch_)) {
        if (bypass_) needs_ramp_up = true;
        cabmodel = static_cast<uint32_t>(cabswitch_);
      }
      bypassed = true;
      //ramp_down = ramp_down_step;
    }
  ...
  if (!bypassed) {
	  ...
	  if (cabswitch_)cabsim->mono_audio(static_cast<int>(n_samples), output, output, cabsim);
  }
}
